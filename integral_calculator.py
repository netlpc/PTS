...
def parse_input_to_sympy(value: str):
    ...
    try:
        expr = sp.sympify(value, locals=SAFE_LOCALS)
        return expr
...
def evaluate_symbolic_function(func_str: str) -> sp.Expr:
    try:
        ...
        expr = sp.sympify(s, locals=SAFE_LOCALS)
...
def gaussian_quadrature_fixed(f, a, b, n=64):
    """
    固定 n 点的 Gauss–Legendre 求积，替代 scipy.integrate.quadrature（已弃用）。
    f: 可向量化函数；[a,b] 有限；n 建议 32/64/128（偶数更佳）
    """
    if not (np.isfinite(a) and np.isfinite(b)):
...
# 顶层用垂直分隔窗，上：控件区（Notebook/History/进度条），下：图像区
...
lang_button = ttk.Combobox(lang_row, textvariable=lang_var, values=["English", "中文", "日本語", "한국어"], state="readonly", width=12)
...
def show_usage_instructions():
    ...
    titles = {"English": "Usage Instructions", "中文": "使用说明", "日本語": "使用説明", "한국어": "사용 안내"}
...
def compute_transform():
    try:
        ...
        clear_plot(plot_ax, plot_canvas)  # 无穷区间不作图
    except Exception as e:
